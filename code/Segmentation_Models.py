...
class Brain_tumor_segmentation_model(object):
    #     def __init__(self, n_epoch=10, n_chan=4, batch_size=128, loaded_model=False, architecture='single', w_reg=0.01, n_filters=[64, 128, 128, 128], k_dims=[2, 5, 5, 3], activation='relu'):
    #         '''
    ...
    #         single.add(Convolution2D(self.n_filters[0], self.k_dims[0], self.k_dims[0], border_mode='valid', W_regularizer=l1l2(
    #             l1=self.w_reg, l2=self.w_reg), input_shape=(self.n_chan, 33, 33)))
    ...
    #                                  activation=self.activation, border_mode='valid', W_regularizer=l1l2(l1=self.w_reg, l2=self.w_reg)))
    #         single.add(BatchNormalization(mode=0, axis=1))
    #         single.add(MaxPooling2D(pool_size=(2, 2), strides=(1, 1)))
    #         single.add(Dropout(0.5))
    ...
    #         model.add_node(Convolution2D(64, 7, 7, border_mode='valid', activation='relu',
    #                                      W_regularizer=l1l2(l1=0.01, l2=0.01)), name='local_c1', input='input')
    #         model.add_node(MaxPooling2D(pool_size=(4, 4), strides=(
    #             1, 1), border_mode='valid'), name='local_p1', input='local_c1')

    #         # local pathway, second convolution/pooling
    #         model.add_node(Dropout(0.5), name='drop_lp1', input='local_p1')
    #         model.add_node(Convolution2D(64, 3, 3, border_mode='valid', activation='relu',
    #                                      W_regularizer=l1l2(l1=0.01, l2=0.01)), name='local_c2', input='drop_lp1')
    #         model.add_node(MaxPooling2D(pool_size=(2, 2), strides=(
    #             1, 1), border_mode='valid'), name='local_p2', input='local_c2')

    #         # global pathway
    #         model.add_node(Convolution2D(160, 13, 13, border_mode='valid', activation='relu',
    #                                      W_regularizer=l1l2(l1=0.01, l2=0.01)), name='global', input='input')
    ...
    #         model.add_node(Convolution2D(5, 21, 21, border_mode='valid', activation='relu',  W_regularizer=l1l2(
    #             l1=0.01, l2=0.01)), name='merge', inputs=['drop_lp2', 'drop_g'], merge_mode='concat', concat_axis=1)
    ...
    #                        name='dense_output', input='flatten')
    ...
    #         single = Sequential()
    #         single.add(Convolution2D(64, 7, 7, border_mode='valid', W_regularizer=l1l2(
    #             l1=0.01, l2=0.01), input_shape=(4, 33, 33)))
    #         single.add(Activation('relu'))
    #         single.add(BatchNormalization(mode=0, axis=1))
    #         single.add(MaxPooling2D(pool_size=(2, 2), strides=(1, 1)))
    #         single.add(Dropout(0.5))
    #         single.add(Convolution2D(nb_filter=128, nb_row=5, nb_col=5, activation='relu',
    ...
    #         single.add(MaxPooling2D(pool_size=(2, 2), strides=(1, 1)))
    #         single.add(Dropout(0.5))
    #         single.add(Convolution2D(nb_filter=128, nb_row=3, nb_col=3, activation='relu',
    #                                  border_mode='valid', W_regularizer=l1l2(l1=0.01, l2=0.01)))
    #         single.add(Dropout(0.25))
    #         single.add(Flatten())
    ...
    #             filepath="./check/bm_{epoch:02d}-{val_loss:.2f}.hdf5", verbose=1)

    #         if self.architecture == 'dual':
    #             self.model_comp.fit([X5_train, X_train], Y_train, batch_size=self.batch_size, nb_epoch=self.n_epoch,
    ...
    #             self.model_comp.fit(data, batch_size=self.batch_size, nb_epoch=self.n_epoch,
    #                                 validation_split=0.1, show_accuracy=True, verbose=1, callbacks=[checkpointer])
    #         else:
    ...
    #             plist.append(p)
    #         patches = np.array(zip(np.array(plist[0]), np.array(
    #             plist[1]), np.array(plist[2]), np.array(plist[3])))
    ...
    #         # intersection of locations defined as tumor_assoc in gt and segmentation
    #         core_edema = unique_rows(combo)
    #         gt_c, seg_c = [], []  # predicted class of each
    #         for i in core_edema:
    #             gt_c.append(gt[i[0]][i[1]])
    #             seg_c.append(seg_full[i[0]][i[1]])
    #         tumor_assoc = len(np.argwhere(np.array(gt_c) ==
    #                                       np.array(seg_c))) / float(len(core))
    #         tumor_assoc_gt = len(np.argwhere(
    #             np.array(gt_c) == np.array(seg_c))) / float(len(gt_tumor))
    ...
    #     patches = PatchLibrary((33, 33), train_data, 50)
    ...
    def __init__(self, is_hgg=None, n_chan=4, loaded_model=False, model_name=None):
        ...
        if not self.loaded_model:
            if self.is_hgg is None:
                raise NameError(
                    'expected boolean True for HGG else False for LGG')
            ...
            self._make_model()
            self._compile_model()
            print('model for {} ready and compiled, waiting for training'.format(
                self.model_name))
        else:
    ...
    def save_segmented_image(self, index, test_img, save=False):
        ...
        if save:
            ...
            try:
                mkdir_p('./results/')
                io.imsave('./results/result' + '_' +
                          str(index) + '.png', sliced_image)
            except:
                io.imsave('./results/result' + '_' +
                          str(index) + '.png', sliced_image)
        else:
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(
        description='Commands to istanciate or load the convolutional neural network')
    ...
    patches = PatchLibrary((33, 33), train_data, result.training_datas)
    X, y = patches.make_training_patches()
...
"""
    if type(result.model_to_load) is int:
        ...
        model = Brain_tumor_segmentation_model(
            loaded_model=True, model_name='./models/' + result.model_to_load)
    ...
    if result.save:
        ...
        if result.hgg:
            ...
            name = 'lgg'
        model.save_model('models/' + name + '_' +
                         str(result.training_datas) + angle + 'result_cnn')

    if result.test:
        tests = glob('test_data/**')
        segmented_images = []
        for index, slice_img in enumerate(tests):
            segmented_images.append(model.save_segmented_image(
                index, test_img=slice_img, save=True))
"""
